...
namespace Sparrow.Xml.Parsers
{
    ...
    /// </summary>
    public sealed class ParamRefParser : CommentNodeParser<ParamRefComment>
    {
        /// <summary>
        ...
        /// The parsed comment.
        /// </returns>
        public override ParamRefComment Parse(ICommentParser parser, XmlNode node)
        {
            Debug.Assert(node.Attributes != null, "Node has no attributes.");
            var attribute = node.Attributes["name"];
            return new ParamRefComment(attribute.InnerText);
        }
    }
}
